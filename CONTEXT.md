...
> 7月の面談スケジュールには含めない。後回しでよい「やりたいことのメモ」。

### YouTube視聴ログ + 英語学習支援アプリ
- アプリ内でYouTube動画を登録・視聴記録する独自ログ機能
  （Google視聴履歴の取得とは別軸。アプリ内で「見た」と記録する形）
- AIが動画内容から英単語フラッシュカードを自動生成
- 単語・動画をCEFRレベル（A1〜C2）ごとに分類
- 将来的にはRoutine Trackerから独立させて
  単体の英語学習アプリ（ImmersionLog的な構想）に発展させる可能性あり
